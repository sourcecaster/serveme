...
const String _clear = '\x1b[999D\x1b[K';
...
void initLogger() {
	try {
		_debugFile = File(config.debugLog).openSync(mode: FileMode.writeOnlyAppend);
	}
	catch (err) {
		error('Unable to write debug log file: $err');
	}
	try {
		_errorFile = File(config.errorLog).openSync(mode: FileMode.writeOnlyAppend);
	}
	catch (err) {
...
Future<void> log(String message, [String color = _green]) async {
	final String time = DateTime.now().toUtc().toString().replaceFirst(RegExp('\\..*'), '');
	...
	console.update();
	if (_debugFile != null) {
		final Future<void> Function(void) func = (_) => _debugFile!.writeString('$time - $message\n');
...
Future<void> debug(String message, [String color = _reset]) async {
	if (config.debug) await log(message, color);
}
...
Future<void> error(String message, [StackTrace? stack]) async {
	...
	console.update();
	if (_errorFile != null) {
		final Future<void> Function(void) func = (_) => _errorFile!.writeString('$time - $message\n${stack.toString()}\n');
...
Future<void> destroyLogger() async {
